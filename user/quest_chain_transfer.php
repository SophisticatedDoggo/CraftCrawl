...
$new_owner_id = filter_var($_POST['new_owner_id'] ?? null, FILTER_VALIDATE_INT);

if (!$chain_id || !$new_owner_id) {
    http_response_code(400);
    echo json_encode(['ok' => false, 'message' => 'Invalid request.']);
    exit();
}

$result = craftcrawl_transfer_chain_ownership($conn, $user_id, $chain_id, $new_owner_id);
...
if ($result['ok'] ?? false) {
    ...
    craftcrawl_send_push_to_user(
        ...
        $new_owner_id,
        'Quest Chain Update',
        ($user['fName'] ?? 'Someone') . ' made you party leader of ' . ($chain['chain_name'] ?? 'a quest chain') . '!',
