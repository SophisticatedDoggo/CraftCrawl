...
$location_id = filter_var($_GET['business_id'] ?? null, FILTER_VALIDATE_INT);
...
$biz_stmt = $conn->prepare("SELECT id FROM locations WHERE id=? AND visibility_status='public_claimed' LIMIT 1");
$biz_stmt->bind_param("i", $location_id);
$biz_stmt->execute();
...
if ($before_dt) {
    $posts_stmt = $conn->prepare("
        ...
        LIMIT ?
    ");
    $posts_stmt->bind_param("isi", $location_id, $before_dt, $fetch_limit);
} else {
    $posts_stmt = $conn->prepare("
        SELECT id, post_type, title, body, created_at, ends_at
        FROM business_posts
        ...
        LIMIT ?
    ");
    $posts_stmt->bind_param("ii", $location_id, $fetch_limit);
}
